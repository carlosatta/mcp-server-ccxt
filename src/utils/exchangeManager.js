...
function createExchangeInstance(name, credentials) {
  const config = credentials
    ? {
        ...
        timeout: 10000, // 10 seconds timeout to prevent transport blocking
      }
    : {
        timeout: 10000, // 10 seconds timeout for public APIs too
      };
  ...
  switch (name) {
    case "coinbase":
      ...
      break;
    case "kraken":
      exchange = new ccxt.kraken(config);
      break;
    case "bitfinex":
      exchange = new ccxt.bitfinex(config);
      break;
    case "bybit":
